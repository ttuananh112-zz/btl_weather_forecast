...
public abstract class GenericRequestTask extends AsyncTask<String, String, TaskOutput> {
    ...
    public GenericRequestTask(Context context, Activity activity, ProgressDialog progressDialog) {
    ...
    protected TaskOutput doInBackground(String... params) {
        ...
        if (params != null && params.length > 0) {
            ...
            else if("suggest".equals(zeroParam)){
                String city = params[1];
//                String day = params[2]; // 0(now) -> 5
                reqParams = new String[]{"suggest", city};
            }
        }
        Log.d("AsyncTask", reqParams[0] + " " + reqParams[1]);
    ...
    private void restorePreviousCity() {
        if (!TextUtils.isEmpty(((MainActivity)activity).recentCityId)) {
            SharedPreferences.Editor editor = PreferenceManager.getDefaultSharedPreferences(context).edit();
            editor.putString("cityId", ((MainActivity)activity).recentCityId);
            editor.commit();
            ((MainActivity)activity).recentCityId = "";
        }
    }
